...
class Actor:
    def __init__(self, sess, S, S_, action_dim, action_bound, tau, lr, f1_units):
        ...
        with tf.variable_scope('actor_replacement'):
            self.replace = [tf.assign(t, self.tau * e + (1 - self.tau) * t) for t, e in
    ...
    def _build_net(self, s, name, trainable):
        ...
        with tf.variable_scope(name):
            ...
            l1 = tf.layers.dense(l1, 400, activation=tf.nn.relu,
            ...
            l2 = tf.layers.dense(l2, 300, activation=tf.nn.relu,
            ...
            with tf.variable_scope('a'):
                actions = tf.layers.dense(l3, self.action_dim, activation=tf.nn.tanh,
    ...
    def _build_net(self, s, a, name, trainable):
        ...
        with tf.variable_scope(name):
            with tf.variable_scope('l1'):
                l1 = tf.layers.batch_normalization(s)
                l1 = tf.layers.dense(l1, 400, kernel_initializer=random_uniform(-f1, f1),
                                     bias_initializer=random_uniform(-f1, f1), trainable=trainable)
                l2 = tf.layers.batch_normalization(l1)
                l2 = tf.layers.dense(l2, 300, kernel_initializer=random_uniform(-f2, f2),
